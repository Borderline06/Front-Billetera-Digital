...
export default function DashboardPage() {
  ...
  return (
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-10">
      <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 border border-gray-200 dark:border-gray-700 shadow-md transition-all duration-300 hover:shadow-lg">
        <h3 className="text-gray-500 dark:text-gray-400 text-sm">Saldo actual</h3>
        <p className="text-3xl font-bold text-gray-900 dark:text-gray-100 mt-2">
          S/ {balance.toLocaleString('es-PE', { minimumFractionDigits: 2 })}
        </p>
      </div>
      <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 border border-gray-200 dark:border-gray-700 shadow-md transition-all duration-300 hover:shadow-lg">
        <h3 className="text-gray-500 dark:text-gray-400 text-sm">Ingresos (Histórico)</h3>
        <p className="text-3xl font-bold text-green-600 dark:text-green-400 mt-2">
          +S/ {ingresos.toLocaleString('es-PE', { minimumFractionDigits: 2 })}
        </p>
      </div>
      <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 border border-gray-200 dark:border-gray-700 shadow-md transition-all duration-300 hover:shadow-lg">
        <h3 className="text-gray-500 dark:text-gray-400 text-sm">Egresos (Histórico)</h3>
      ...
      <button className="flex items-center gap-2 text-indigo-900 font-medium hover:text-indigo-700 dark:text-indigo-400 dark:hover:text-indigo-300">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-inner">
        <h3 className="text-gray-800 dark:text-gray-100 text-base font-semibold">Actividad</h3>
        <div className="flex items-baseline gap-2 mt-2">
          <p className="text-3xl font-bold text-gray-900 dark:text-gray-100">
            S/ {balance.toLocaleString('es-PE', { minimumFractionDigits: 2 })}
          </p>
        </div>
        <p className="text-gray-500 dark:text-gray-400 text-sm mb-4">Últimos 30 días</p>
        ...
        {dailyBalance.length > 0 ? (
          <ResponsiveContainer width="100%" height={250}>
            <AreaChart data={dailyBalance}>
              ...
              <Tooltip
                contentStyle={{
                  backgroundColor: '#1f2937', // gris oscuro para dark
                  borderRadius: '8px',
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-inner">
        <h3 className="font-semibold text-lg mb-4 text-gray-800 dark:text-gray-100">Últimas Transacciones</h3>
        {transactions.length > 0 ? (
          <ul className="space-y-4">
            {transactions.slice(0, 8).map((tx) => {
              const isNegative =
                tx.type.includes('SENT') || tx.type === 'WITHDRAWAL' || tx.type === 'TRANSFER';
              ...
              return (
                <li key={tx.id} className="flex items-center justify-between">
                  ...
                  <div className="flex-1 ml-4">
                    <p className="text-gray-800 dark:text-gray-100 font-medium capitalize">
                  ...
                  <div className="text-right">
                    ...
                    <span
                      className={`text-xs px-2 py-1 rounded-full font-medium ${
                        statusColors[status] || 'bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-200'
                      }`}
                    >
                      {status}
                    </span>
          ...
          <p className="text-gray-500 dark:text-gray-400 text-center py-8">No hay transacciones.</p>
        )}
      </div>
    </div>
  </div>
